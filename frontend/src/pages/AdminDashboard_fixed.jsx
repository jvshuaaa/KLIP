...
export default function AdminDashboard() {
  ...
  const [surveyImages, setSurveyImages] = useState({ survey_image_1: null, survey_image_2: null, survey_image_3: null, survey_image_4: null });
  const [surveyImageUploading, setSurveyImageUploading] = useState({});
  const [surveyImageError, setSurveyImageError] = useState("");
  const [surveyImageFiles, setSurveyImageFiles] = useState({});
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      setSurveyImages({
        survey_image_1: siteData.survey_image_1 || null,
        survey_image_2: siteData.survey_image_2 || null,
        survey_image_3: siteData.survey_image_3 || null,
        survey_image_4: siteData.survey_image_4 || null
      });
    } catch (error) {
  ...
  const fetchSurveyImages = async () => {
    try {
      ...
      const data = res.data || {};
      setSurveyImages({
        survey_image_1: data.survey_image_1 || null,
        survey_image_2: data.survey_image_2 || null,
        survey_image_3: data.survey_image_3 || null,
        survey_image_4: data.survey_image_4 || null
      });
    } catch (err) {
      console.error("Failed to load survey images:", err);
    }
  };

  const handleSurveyImageFileChange = (key, file) => {
    setSurveyImageFiles(prev => ({ ...prev, [key]: file }));
  };

  const handleSurveyImageUpload = async () => {
    const fd = new FormData();
    
    Object.entries(surveyImageFiles).forEach(([key, file]) => {
      if (file) {
        fd.append(key, file);
      }
    });
    ...
    if (fd.entries().next().done) {
      setSurveyImageError("Pilih setidaknya satu gambar untuk diupload.");
      return;
    }

    setSurveyImageUploading({ all: true });
    setSurveyImageError("");
    
    try {
      const res = await api.post("/admin/survey-images", fd, { 
        headers: { "Content-Type": "multipart/form-data" } 
      });
      
      setSurveyImages(prev => ({ ...prev, ...res.data.updated_images }));
      setSurveyImageFiles({});
      setSurveyImageError("");
      alert("Gambar survey berhasil diperbarui!");
    } catch (err) {
      setSurveyImageError(err.response?.data?.message || err.response?.data?.error || "Gagal mengupload gambar survey.");
    } finally {
      setSurveyImageUploading({ all: false });
    }
  };
        ...
        <main className="flex-1 min-w-0 space-y-6">
          ...
          {view === "menu" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <button
                onClick={() => { fetchAdminData(); setView("consultation"); }}
              ...
              {/* Manajemen User */}
              <button
                onClick={() => { fetchAdminData(); setView("users"); }}
              ...
              {/* Manajemen Dokumen */}
              <button
                onClick={() => { fetchAdminData(); setView("documents"); }}
              ...
              {/* Survey Images */}
              <button
                onClick={() => { fetchSurveyImages(); setView("survey_images"); }}
              ...
              >
                ...
                <div className="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center mb-3 group-hover:bg-purple-600 transition-colors">
                  <Image className="w-5 h-5 text-purple-600 group-hover:text-white transition-colors" />
                </div>
                <p className="font-semibold text-gray-800 text-sm mb-1">Kelola Gambar Survey</p>
                <p className="text-xs text-gray-500 mb-3">Upload dan atur 4 foto galeri survey.</p>
                <span className="text-xs bg-purple-50 text-purple-700 border border-purple-200 px-2 py-0.5 rounded-full font-medium">4 Images</span>
              </button>

              {/* Gambar Halaman */}
              <button
                onClick={() => { fetchAdminData(); setView("site_images"); }}
          ...
          {view === "survey_images" && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Gambar Saat Ini</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[1, 2, 3, 4].map((num) => (
                      <div key={num} className="relative group">
                        <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden border-2 border-gray-200">
                          {surveyImages[`survey_image_${num}`] ? (
                            <img
                              src={`http://localhost:8000/${surveyImages[`survey_image_${num}`]}`}
                              alt={`Survey Image ${num}`}
                        ...
                        <div className="mt-2 text-center">
                          <span className="text-sm font-medium text-gray-700">Gambar {num}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Upload Section */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Upload Gambar Baru</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[1, 2, 3, 4].map((num) => (
                      <div key={num} className="space-y-2">
                        ...
                        <div
                          ...
                          onClick={() => document.getElementById(`survey-image-${num}`).click()}
                        >
                          {surveyImageFiles[`survey_image_${num}`] ? (
                            <div className="space-y-2">
                              <div className="text-green-600 font-medium">
                                {surveyImageFiles[`survey_image_${num}`].name}
                              </div>
                              <div className="text-xs text-gray-500">
                                {(surveyImageFiles[`survey_image_${num}`].size / 1024 / 1024).toFixed(2)} MB
                              </div>
                            </div>
                          ) : (
                            <div className="space-y-2">
                        ...
                        <input
                          ...
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleSurveyImageFileChange(`survey_image_${num}`, e.target.files[0])}
                          className="hidden"
                        />
                      </div>
                    ))}
                ...
                {surveyImageError && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <p className="text-red-700 text-sm">{surveyImageError}</p>
                  </div>
                )}
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={surveyImageUploading.all}
